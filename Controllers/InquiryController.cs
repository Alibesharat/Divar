...
public class InquiryController : Controller
{
    ...
    public InquiryController(
        ...
        IOptions<DivarSetting> divarSetting,
    ...
    public async Task<IActionResult> GetPostDataAsync(string scope, string state)
    {
        ...
        {
            var accessToken = await _divarService.ExchangeCodeForAccessTokenAsync(
                "code",
                _divarSetting.RedirectUrl
            );
            Console.WriteLine("AccessToken", accessToken);
            postData = await _divarService.GetPostDataAsync(postToken, accessToken);
            if (postData == null)
                throw new ArgumentNullException(
                    "Post Data is null or can not handshake with divar"
                );
        }
        else
        {
            ...
            {
                Token = "new",
                Data = new Data() { Title = "درخواست جدید" },
            };
        }
    ...
    )
    {
        ...
        string message =
            $"{reservation.FullName} عزیز  سفارش شما با کد رهگیری  {reservation.TrackingCode} با موفقیت ثبت شد ، کارشناس {expert.FullName} جهت هماهنگی بیشتر با شما تماس میگیرد . ";
